...
### 畫面 E:儀表板(含台灣地圖)
```
設計「儀表板」,最上方主視覺是台灣縣市地圖(內嵌簡化 SVG path 繪製
22 縣市,不要載外部資源),其餘為漏斗圖(新名單→成交)、各產業名單數
長條圖、資料完整度(統編比對率/LINE覆蓋率)環形圖、被冷落的A級客戶
清單。圖表 Recharts 風格,桌機=地圖佔左 1/2、右側 2×2 圖卡;手機=
地圖置頂直向堆疊。

台灣地圖規格:
- 兩種模式切換(Tab):
  1.「名單分布」:縣市底色深淺=名單數量(奶油白→霧松綠五階),
    點擊縣市顯示該縣市各產業名單組成(底部抽屜或側欄)
  2.「代工插旗」:已有成交 OEM 客戶的縣市放圓點旗標,
    旗標顏色=客戶產業類型,同縣市多類型則並排小圓點;
    未插旗縣市維持淡灰,一眼看出空白市場
- 產業色票(低飽和,全系統一致):
  養生館/按摩 #6B8F71、宮廟 #D9B68C、長照 #A8BCA1、
  禮儀 #5E6B5F、寵物 #C98A6B、旅宿/溫泉 #8FA3B0、其他 #B8B2A5
- 地圖旁固定圖例;hover(桌機)/點擊(手機)顯示縣市名+數字
- 縣市文字標籤 ≥13px;離島(澎金馬)縮排在右下角小框
```
...
2. 比對中心:低信心配對佇列,左右並排「Places 資料 vs 政府名冊資料」
   差異高亮,確認/拒絕按鈕;另含「別名表」分頁
   (招牌名↔登記名對照,可新增/編輯)
```

### 畫面 G:擴充中心(後台,桌機 only)
```
設計「擴充中心」頁(桌機 only):
- 上方統計卡:待擴充 N 筆 / 擴充成功率 / Email・LINE 覆蓋率
- 「待擴充」表格:品牌/官網/最後嘗試時間,勾選後「批次執行」按鈕
- 「失敗清單」分頁:失敗原因 Badge(無官網/粉專反爬/逾時),
  每列有「人工查找」按鈕展開手動填入 Email/LINE/FB/IG 的表單
- 執行中顯示溫和的進度指示(霧松綠進度條,不要轉圈警示感)
```

### 畫面 H:客情維護管理(成交客戶)
```
設計「客情維護」頁(桌機+手機 RWD):
- 成交客戶卡片列表:品牌/分級 Badge(A燕麥金・B鼠尾草・C淡灰)/
  距上次互動天數/下次回購倒數/健康度圓點(綠・黃・陶土)
- 點開客戶:分級與拜訪週期設定、回購週期(天)與上次出貨日編輯、
  三節送禮紀錄(年份+內容)、新增客情任務按鈕
- 頂部分頁:全部/被冷落(健康度黃紅)/本月三節任務
- 手機:卡片直列,健康度排序置頂
```

### 畫面 I:登入頁
```
設計極簡登入頁:置中卡片、HeroHerb 標誌佔位、Email+密碼(Supabase
Auth)、霧松綠按鈕、奶油白背景,加一句品牌語(草本溫潤調性)。
```
...
```json
{
  ...
  "care_customers": [
    {"brand":"青松健康","tier":"A","last_contact_days":3,"reorder_in":6,"health":"green"},
    {"brand":"6星集","tier":"A","last_contact_days":41,"reorder_in":null,"health":"red"},
    {"brand":"滋和堂","tier":"B","last_contact_days":18,"reorder_in":22,"health":"yellow"}
  ],
  "map_counties": [
    {"county":"台北市","leads":86,"won_types":["養生館","禮儀"]},
    {"county":"新北市","leads":74,"won_types":["越式洗髮"]},
    {"county":"桃園市","leads":41,"won_types":[]},
    {"county":"台中市","leads":58,"won_types":["宮廟","長照"]},
    {"county":"彰化縣","leads":19,"won_types":["長照"]},
    {"county":"台南市","leads":33,"won_types":[]},
    {"county":"高雄市","leads":47,"won_types":["養生館"]},
    {"county":"宜蘭縣","leads":12,"won_types":["旅宿"]},
    {"county":"花蓮縣","leads":8,"won_types":[]}
  ],
  "enrich_queue": [
    {"brand":"悅禾莊園SPA","website":"yuehe.tw","status":"pending"},
    {"brand":"小林越式洗髮","website":null,"status":"failed","reason":"無官網"}
  ],
  "care_today": [
...
A 名單總覽 → D 今日跟進(手機)→ B 品牌詳情 → C 進度看板
→ H 客情維護 → E 儀表板 → F 採集+比對 → G 擴充中心 → I 登入
