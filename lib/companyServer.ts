...
export async function getCompany(): Promise<CompanyInfo> {
  const cfg = await getCfgMany([...Object.values(COMPANY_KEYS), "COMPANY_SALES_REPS"]);
  const pick = (k: keyof typeof COMPANY_KEYS) => {
    const v = cfg[COMPANY_KEYS[k]];
    return v && v.trim() !== "" ? v : (DEFAULT_COMPANY[k] as string);
  };
  const salesReps = (cfg.COMPANY_SALES_REPS || "")
